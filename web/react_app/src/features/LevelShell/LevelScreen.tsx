...
import type { getLevelCombatDeltas } from "@data/vessels/vessels";
import "./style.css";

type CombatDeltas = ReturnType<typeof getLevelCombatDeltas>;
...
/**
 * LevelScreen — game logic + presentational layout.
 * Tap-grid mini-game with timer, HUD, health bar.
 * No store access. No dispatch. Calls onLevelComplete when game ends.
 */
export default function LevelScreen({
    ...
    const endGame = (msg: string, survived: boolean) => {
        ...
        setTimeout(() => {
            onLevelComplete(survived, stateRef.current.points);
        }, 2000);
    };
        ...
        if (litCell && idx === litCell.idx) {
            ...
            if (stateRef.current.tapsHit >= stateRef.current.tapsNeeded) {
                if (stateRef.current.missCount === 0 && combatDeltas.perfectLevelHeal > 0) {
                    stateRef.current.health = Math.min(stateRef.current.health + combatDeltas.perfectLevelHeal, maxHealth);
            ...
            const forgiven =
                combatDeltas.missForgivenessEveryN !== null &&
        ...
        <div className="level-shell-wrapper">
            <div className="top-bar">
                <div>
                    ...
                    <br />
                    <span className="top-bar-phase" style={{ color: "#C04050" }}>
                        {levelNames[depth - 1] || `Depth ${depth}`}
                    </span>
                </div>
                ...
                <div className="game-slot">
                    {gameOverMsg && (
                        <div className="system-message">
                            <span
                                style={{
                                    color: gameOverMsg.includes("DESTROYED") || gameOverMsg.includes("EXPIRED") ? "#C04050" : "#D4A843",
                                }}
                            >
                                {gameOverMsg}
                            </span>
                    ...
                    <div className="tap-area">
                        {Array.from({ length: 9 }).map((_, idx) => {
                            ...
                            return (
                                <div key={idx} className={className} onClick={() => handleTap(idx)}>
                                    {content}
                                </div>
                            );
                        })}
