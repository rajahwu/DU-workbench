...
const phaseData = {
    ...
    level: { name: "Level", color: "#C04050", summary: "Active gameplay. A flexible container that loads different mini-game types. Points earned here determine which Doors become available.", theology: "The Depth tests. It does not explain itself. The Vessel endures or it does not." },
...
/**
 * StagingScreen — presentational layout.
 * Phase architecture diagram, live engine stats, loop visualization.
 * No store access. No dispatch. Pure props + callbacks.
 */
export default function StagingScreen({ activePhaseId, onSelectPhase, onInitiateDraft, runMeta }: StagingScreenProps) {
    const activeData = phaseData[activePhaseId as keyof typeof phaseData];
            ...
            <div className="detail-panel">
                ...
                <div className="detail-col">
                    ...
                    <div style={{ fontSize: "8px", color: "#2A2D38", marginTop: "8px", lineHeight: 1.8 }}>
                        {["draft", "level", "door"].includes(activePhaseId)
                            ? "Draft → Level → Door repeats as the player descends through Depth. Each cycle the cards change, the difficulty scales, and the parity shifts."
                            : "Core checkpoint. Progression limits applied here."}
                    </div>
                </div>

                <div className="detail-col" style={{ justifyContent: "flex-end" }}>
                    <button className="draft-button" onClick={onInitiateDraft}>
                        INITIATE DRAFT
                    </button>
                </div>
            </div>
